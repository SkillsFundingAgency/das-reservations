...
public class WhenCallingGetApprenticeshipTraining
{
    ...
    public async Task Then_It_Returns_The_Apprenticeship_Training_View_With_Mapped_Values(
        ...
        [NoAutoProperties] ReservationsController controller)
    {
        var expectedStartDates = GetMockTrainingDates().ToList();
        var firstDayOfCurrentMonth = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1);
        var expectedPastDate = expectedStartDates.Where(d => d.StartDate < firstDayOfCurrentMonth).OrderBy(d => d.StartDate).LastOrDefault();
        var expectedPossibleDates = expectedStartDates.Where(d => d.StartDate >= firstDayOfCurrentMonth).OrderBy(d => d.StartDate).Select(m => new TrainingDateViewModel(m)).ToList();
        ...
        mockEncodingService
            ...
            .Returns(accountLegalEntityId);
        mockStartDateService
            .Setup(service => service.GetTrainingDates(accountLegalEntityId, It.IsAny<string>()))
        ...
        viewModel.PossibleStartDates.Should().BeEquivalentTo(expectedPossibleDates);
        ...
        if (expectedPastDate != null)
        {
            viewModel.PastStartDate.Should().NotBeNull();
            viewModel.PastStartDate.Id.Should().Be($"{expectedPastDate.StartDate:yyyy-MM}");
        }
        else
        {
            viewModel.PastStartDate.Should().BeNull();
        }
    }
